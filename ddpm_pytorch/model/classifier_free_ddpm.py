...
class GaussianDDPMClassifierFreeGuidance(pl.LightningModule):
    ...
    def __init__(self, denoiser_module: nn.Module, T: int,
        ...
        self.alphas_hat: torch.FloatTensor = self.var_scheduler.get_alpha_hat().to(device)
        self.alphas: torch.FloatTensor = self.var_scheduler.get_alphas().to(device)
        self.betas = self.var_scheduler.get_betas().to(device)
        self.betas_hat = self.var_scheduler.get_betas_hat().to(device)
    ...
    def training_step(self, batch, batch_idx):
        return self.compute_batch_loss(batch, batch_idx, 'train')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return self.compute_batch_loss(batch, batch_idx, 'valid')

    def compute_batch_loss(self, batch, batch_idx, dataset: Literal['train', 'valid']) -> torch.Tensor:
        """
        ...
        loss = self.mse(eps, pred_eps).view(X.shape[0], -1).mean(dim=1) # compute the MSE between the predicted noise and the real noise
    ...
    def on_fit_start(self) -> None:
        ...
        self.betas_hat = self.betas_hat.to(device)
    ...
    def generate(self, batch_size: Optional[int] = None, c: Optional[torch.Tensor] = None, T: Optional[int] = None,
        ...
        is_c_none = c is None
        if is_c_none:
            c = torch.zeros(batch_size, self.num_classes, device=torch.device('cuda'))
        if get_intermediate_steps:
            steps = []
        z_t = torch.randn(batch_size, self.input_channels,  # start with random noise sampled from N(0, 1)
                          self.width, self.height, device=torch.device('cuda'))
        for t in range(T - 1, 0, -1):
            if get_intermediate_steps:
                steps.append(z_t)
            t = torch.LongTensor([t] * batch_size).to(torch.device('cuda')).view(-1, 1)
