...
from ddpm_pytorch.model.unet_class import UNetTimeStepClassConditioned
from ddpm_pytorch.variance_scheduler.cosine import CosineScheduler
...
parser.add_argument('--T', default=1000, type=int, metavar='N',
                    help='the amount of noising steps')
parser.add_argument('--w', default=0.3, type=float,
                    help='strength of class guidance')
parser.add_argument('-p', '--p_uncond', default=0.2, type=float,
                    help='probability of train a batch without class conditioning')
parser.add_argument('--logging_freq', default=1000, type=float,
                    help='frequency of logging loss function during training')
parser.add_argument('--v', default=0.2, type=float,
                    help='generative variance hyper-parameter')

parser.add_argument('--var_scheduler', default='cos', type=str,
                    help='variance scheduler in the forward pass')
parser.add_argument('--accelerator', default='gpu', type=str,
                    help='pim_memory in dataloader')

# Denoiser Architecture:
parser.add_argument('--denoiser', default='unet')
parser.add_argument('--channels', default=[1, 32, 64, 64, 96], type=list[int],
                    help='generative variance hyper-parameter')
parser.add_argument('--kernel_sizes', default=[3, 3, 3, 3], type=list[int],
                    help='generative variance hyper-parameter')
parser.add_argument('--strides', default=[1, 1, 1, 1], type=list[int],
                    help='generative variance hyper-parameter')
parser.add_argument('--paddings', default=[1, 1, 1, 1], type=list[int],
                    help='generative variance hyper-parameter')
parser.add_argument('--p_dropouts', default=[0.1, 0.1, 0.1, 0.1], type=list[float],
                    help='generative variance hyper-parameter')
parser.add_argument('--time_embed_size', default=100, type=int,
                    help='generative variance hyper-parameter')  # did not found this hp on the paper
parser.add_argument('--downsample', default=True, type=bool,
                    help='generative variance hyper-parameter')
parser.add_argument('--class_embed_size', default=3, type=int,
                    help='generative variance hyper-parameter')
parser.add_argument('--assert_shapes', default=False, type=bool,
                    help='generative variance hyper-parameter')
...
parser.add_argument('--lr_min', default=1.0e-5, type=float,
...
parser.add_argument('--lr_max', default=1.0e-2, type=float,
...
parser.add_argument('--use_curriculum', default=True, action='store_true',
                    help='if using curriculum learning or not')
parser.add_argument('--use_mean_teacher', default=True, action='store_true',
                    help='maintain a time moving average of the model (ensemble over time) as a teacher for consistency regularization')
parser.add_argument('--use_random_subsample', default=True, action='store_true',
                    help='random subsample a subset before running centrality max to further select samples')
parser.add_argument('--use_centrality', default=True, action='store_true',
...
def main():
    ...
    if args.dataset == 'stl10' or args.dataset == 'svhn':
        ...
        trainset = dataloader(root=args.data_path + '/' + args.dataset, train=True, download=True,
                              transform=transform_train2)
        testset = dataloader(root=args.data_path + '/' + args.dataset, train=False, download=False,
                             transform=transform_test)
        trainset0 = dataloader(root=args.data_path + '/' + args.dataset, train=True, download=True,
                               transform=transform_test)
        # trainset.data, trainset.targets = trainset.data[:10], trainset.targets[:10]
        # testset.data, testset.targets = testset.data[:10], testset.targets[:10]
        # trainset0.data, trainset0.targets = trainset0.data[:10], trainset0.targets[:10]
        # if args.num_aug > 0:
        #     testset_aug = dataloader(root=args.data_path + '/' + args.dataset, train=False, download=False,
        #                              transform=transform_train)
        #     trainset_aug = dataloader(root=args.data_path + '/' + args.dataset, train=True, download=False,
        #                               transform=transform_train)
        ...
        if args.use_noisylabel:
            ...
            target_copy, actual_noise_rate = noisify(train_labels=np.asarray(target_copy),
                                                     noise_type=args.label_noise_type, noise_rate=args.label_noise_rate,
                                                     random_state=args.random_state, nb_classes=num_classes)
        ...
        if use_cuda:
            target_copy = torch.cuda.LongTensor(target_copy.cuda())
        else:
    ...
    testloader = data.DataLoader(testset, batch_size=args.train_batch, shuffle=False, num_workers=args.workers,
                                 pin_memory=args.accelerator)
    trainloader_val = data.DataLoader(trainset0, batch_size=args.test_batch, shuffle=False, num_workers=args.workers,pin_memory=args.accelerator)
    if args.num_aug > 0:
        testloader_aug = data.DataLoader(testset_aug, batch_size=args.test_batch, shuffle=False,
                                         num_workers=args.workers)
        trainloader_aug = data.DataLoader(trainset_aug, batch_size=args.test_batch, shuffle=False,
                                          num_workers=args.workers)
    else:
        testloader_aug = None
    ...
    print("==> creating model '{}'".format(args.denoiser))
    if args.dataset == 'fmnist' or args.dataset == 'kmnist' or args.dataset == 'mnist':
        model = torch.nn.DataParallel(UNetTimeStepClassConditioned(args.channels, args.kernel_sizes, args.strides, args.paddings,
                                           args.downsample, args.p_dropouts, args.time_embed_size, num_classes,
                                           args.class_embed_size, args.assert_shapes).cuda(), device_ids=device_ids)
        if args.use_mean_teacher:
            ema_model = torch.nn.DataParallel(UNetTimeStepClassConditioned(args.channels, args.kernel_sizes, args.strides, args.paddings,
                                                   args.downsample, args.p_dropouts, args.time_embed_size, num_classes,
                                                   args.class_embed_size, args.assert_shapes).cuda(),
        ...
        model = torch.nn.DataParallel(wrn(input_shape=input_shape, num_classes=num_classes, depth=args.depth,
                                          widen_factor=args.widen_factor, repeat=args.repeat, dropRate=args.drop,
                                          bias=True).cuda(), device_ids=device_ids)
        if args.use_mean_teacher:
            ema_model = torch.nn.DataParallel(wrn(input_shape=input_shape, num_classes=num_classes, depth=args.depth,
                                                  widen_factor=args.widen_factor, repeat=args.repeat,
                                                  dropRate=args.drop, bias=True).cuda(), device_ids=device_ids)
            for param in ema_model.parameters():
                param.detach_()
            init_as_ema(ema_model, model)
        else:
    ...
    criterion = nn.CrossEntropyLoss() # We use MSE in the loss calculator. This is a dummy one.
    if args.var_scheduler == 'cos':
        variance_scheduler = CosineScheduler(args.T)
    else:
        assert "Add other options around line 434."
    loss_calculator = GaussianDDPMClassifierFreeGuidance(model, args.T,
                                                         args.w, args.p_uncond, input_shape[2],
                                                         input_shape[3], input_shape[1], num_classes,
                                                         args.logging_freq, args.v, variance_scheduler)
    ema_loss_calculator = GaussianDDPMClassifierFreeGuidance(ema_model, args.T,
                                                         args.w, args.p_uncond, input_shape[2],
                                                         input_shape[3], input_shape[1], num_classes,
                                                         args.logging_freq, args.v, variance_scheduler)
    ...
    batches_this_episode = np.ceil(float(n_train) / float(args.train_batch)) * (
            args.schedule[schedule_idx + 1] - args.schedule[schedule_idx] + 1)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == args.schedule[min(schedule_idx + 1, len(args.schedule) - 1)]:
            ...
            if args.use_curriculum:
                ...
                batches_this_episode = batches_per_epoch * (
                        args.schedule[schedule_idx + 1] - args.schedule[schedule_idx]) + np.ceil(
                    float(n_train) / float(args.train_batch))

                # one-pass inference of all training samples
                train_loss, _, all_loss, _, _, train_fea, _, _ = test_prime(loss_calculator if ema_model is None else ema_loss_calculator, trainloader_val,
                                                                                                 None,
                                                                                                 model if ema_model is None else ema_model,
                                                                                                 criterion, epoch,
                                                                                                 args.num_aug, use_cuda)
                # # save dynamics
                # if args.save_dynamics:
                ...
                if args.use_centrality and epoch <= args.schedule[args.explore_cut_episode]:
                    ...
                    if args.use_kernel_centrality:
                        ...
                        train_sims, centrality = compute_sims(np.clip(train_fea, 1.0e-10, 1.0e+10), sigma=20.,
                                                              metric='cos')
                    else:
                ...
                batches_this_episode = batches_per_epoch * (
                        args.schedule[schedule_idx + 1] - args.schedule[schedule_idx])
        ...
        if (not args.use_curriculum) or (epoch < args.schedule[1] or schedule_idx > 0 and epoch == args.schedule[
            min(schedule_idx, len(args.schedule) - 1)]):
            ...
            trainloader = data.DataLoader(Subset(trainset, indices), batch_size=args.train_batch, shuffle=False,
                                          num_workers=args.workers, pin_memory=args.accelerator)
            if epoch < args.schedule[1]:
                train_loss, _, all_loss_per_sample, _, _, all_res_per_sample, all_pred_per_sample, batches_counter, pace_delta = train_one_epoch(
                    loss_calculator, ema_loss_calculator, trainloader, model, ema_model, criterion, optimizer, batches_counter, batches_this_episode,
                    use_cuda, args.ema_decay, 0., 0., 0., lr_schedule='linear')
            elif args.use_curriculum and schedule_idx > 0 and epoch == args.schedule[
                min(schedule_idx, len(args.schedule) - 1)]:
                train_loss, _, all_loss_per_sample, _, _, all_res_per_sample, all_pred_per_sample, batches_counter, pace_delta = train_one_epoch(
                    loss_calculator, ema_loss_calculator, trainloader, model, ema_model, criterion, optimizer, batches_counter, batches_this_episode,
                    use_cuda, args.ema_decay, 0., 0., 0., lr_schedule='cosine', mul=args.batch_mul, mse_loss=True)
            else:
                train_loss, _, all_loss_per_sample, _, _, all_res_per_sample, all_pred_per_sample, batches_counter, pace_delta = train_one_epoch(
                    loss_calculator, ema_loss_calculator, trainloader, model, ema_model, criterion, optimizer, batches_counter, batches_this_episode,
                    use_cuda, args.ema_decay, 0., 0., args.alpha, lr_schedule='cosine')

            all_pace += pace_delta
            all_res_per_sample[indices] = all_res_per_sample.clone()
            all_pred_per_sample[indices] = all_pred_per_sample.clone()
            if epoch > 0:
                # time difference of logits

                diff_preds = all_pred_per_sample - all_pred_old

                # compute residual
                res = all_res_per_sample.clone()

                # update docl score: acc_score
                if epoch == 1:
                    acc_score = (res * diff_preds).sum(dim=1)  # .abs()
                else:
                    rc = torch.from_numpy(np.power(all_pace, 1.e-1)).float().to(diff_preds.get_device())
                    acc_score = args.ema_decay * acc_score + (1. - args.ema_decay) * (res * diff_preds).sum(dim=1) / (
                            (n_train / rc.sum()) * rc)


            # store the current feedback for next iteration comparison
            all_pred_old = all_pred_per_sample.clone()
            all_res_old = all_res_per_sample.clone()
        ...
        if args.use_curriculum and epoch >= args.schedule[1]:
            ...
            print('\nEpoch: [%d | %d] LR: %f, sampled, subset: %d, %d' % (
                epoch + 1, args.epochs, state['lr'], k0, subset_size))
            ...
            if args.use_centrality:
                ...
                if args.use_random_subsample:
                    ...
                    train_subset = np.argpartition(mod * rand_prob[rand_train_set] + centrality[rand_train_set],
                ...
                centrality_estimate, modular_estimate = centrality[rand_train_set].sum(), rand_prob[
                    rand_train_set].sum()
            else:
                rand_train_set = np.random.choice(n_train, k0, p=rand_prob, replace=False)
            ...
            trainselectloader = data.DataLoader(trainset, batch_size=args.train_batch,
                                                sampler=SubsetSampler(rand_train_set), num_workers=args.workers, pin_memory=args.accelerator)
            if rand_train_set_old is not None:
                print('new samples selected comparing to previous epoch:',
                      len(np.setdiff1d(rand_train_set, rand_train_set_old, assume_unique=True)))
            rand_train_set_old = rand_train_set.copy()
            old_lr = args.lr
            train_loss1, _, loss_rest, _, _, res_rest, pred_rest, batches_counter, pace_delta2 = train_one_epoch(
                loss_calculator, ema_loss_calculator, trainselectloader, model, ema_model, criterion, optimizer, batches_counter, batches_this_episode,
                use_cuda, args.ema_decay, args.consistency, args.contrastive, args.alpha, lr_schedule='cosine')
            ...
            diff_preds = pred_rest - all_pred_old[rand_train_set]
            res = res_rest.clone()
            rc = torch.from_numpy(np.power(all_pace[rand_train_set], 1.e-1)).float().to(diff_preds.get_device())
            acc_score[rand_train_set] = args.ema_decay * acc_score[rand_train_set] + (1. - args.ema_decay) * (
                    res * diff_preds).sum(dim=1) / ((len(rand_train_set) / rc.sum()) * rc)

            # store the current feedback for next iteration comparison
            all_res_old[rand_train_set] = res_rest.clone()
            all_pred_old[rand_train_set] = pred_rest.clone()
        ...
        test(loss_calculator, testloader)
        # test_loss, test_acc, all_loss_test, all_correct_test, _, test_fea, all_pred_test, _ = test(testloader, None,
        #                                                                                            model, criterion,
        #                                                                                            epoch, 0, use_cuda)
        # if args.use_mean_teacher:
        #     ema_test_loss, ema_test_acc, _, _, _, _, _, _ = test(testloader, None, ema_model, criterion, epoch, 0,
        #                                                          use_cuda)
        # if args.save_dynamics:
    ...
    #     if args.use_mean_teacher:
    #         result_iter = np.asarray(
    #             [epoch, train_loss, test_loss, train_acc, test_acc, ema_test_loss, ema_test_acc, epoch_time[epoch],
    #              train_sample_num])
    #     else:
    #         result_iter = np.asarray(
    #             [epoch, train_loss, test_loss, train_acc, test_acc, epoch_time[epoch], train_sample_num])
...
def train_one_epoch(loss_calculator, ema_loss_calculator, trainloader, model, ema_model, criterion, optimizer, step, total_steps, use_cuda, ema_decay,
                    consistency=20., contrastive=1., alpha=0., lr_schedule='cosine', mul=1., mse_loss=True,
                    dict_size=10):
    # switch to train mode
    loss_calculator.denoiser_module.train()
    ...
    for batch_idx, ((inputs, ema_inputs), targets) in enumerate(trainloader):
        ...
        if alpha > 0 and bs > 1:
            ...
            inputs, targets, targets_b = torch.autograd.Variable(inputs), torch.autograd.Variable(
                targets), torch.autograd.Variable(targets_b)
        else:
            inputs, targets = torch.autograd.Variable(inputs), torch.autograd.Variable(targets)

        # compute
        eps, pred_eps, loss = loss_calculator.training_step(batch, batch_idx)
        outfea = pred_eps.view(pred_eps.shape[0], -1)
        if ema_model is not None:
            if use_cuda:
                ema_inputs = ema_inputs.cuda()
            with torch.no_grad():
                ema_batch = (ema_inputs, targets)
                ema_eps, ema_pred_eps, ema_loss = ema_loss_calculator.training_step(ema_batch, batch_idx)
                ema_fea = ema_pred_eps.view(ema_pred_eps.shape[0], -1)
            all_pred = all_pred + (ema_pred_eps.view(bs, -1),)
            all_residual = all_residual + ((ema_eps-ema_pred_eps).view(bs, -1),)
            ...
            if contrastive > 0:
                ...
                cont_logprob = -logsoftmax(
                    torch.mm(outfea / outfea.norm(p=2, dim=1).unsqueeze(dim=1), ema_fea_dict.t()) / 0.1)
        ...
        if alpha > 0 and bs > 1:
            prob = softmax(pred_eps - pred_eps.max(dim=1).values[:, None])
            prob1 = 0.5 * ((prob * prob).sum(dim=1) + 1.)
            if mse_loss:
                loss_b = prob1 - prob.gather(1, targets_b.view(-1, 1)).squeeze()
            else:
                loss_b = -logsoftmax(pred_eps).gather(1, targets_b.view(-1, 1)).squeeze()
            loss_b = prob1 - prob.gather(1, targets_b.view(-1, 1)).squeeze()
            ...
            if ema_model is not None:
                if consistency > 0:
                    cons_loss = lam * cons_loss + (1. - lam) * F.mse_loss(outfea, ema_fea[mixup_index],
    ...
    all_residual = torch.cat(all_residual, 0)
    all_pred = torch.cat(all_pred, 0)

    return (losses.avg, None, all_loss, None, None, all_residual, all_pred, step - 1, pace)

def test(loss_calculator, testloader):
    loss_calculator.denoiser_module.eval()
    for batch_idx, (inputs, targets) in enumerate(testloader):
        if use_cuda:
            inputs, targets = inputs.cuda(), targets.cuda()
        batch = (inputs, targets)
        _, _, loss = loss_calculator.validation_step(batch, batch_idx)

def test_prime(loss_calculator, testloader, augloader, model, criterion, epoch, num_aug, use_cuda):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        def get_output(dl_iter):
            ...
            # compute output
            with torch.no_grad():
                _, outputs, _ = loss_calculator.validation_step(batch, batch_idx)
        ...
        with torch.no_grad():
            # compute output
            eps, outputs, loss = loss_calculator.validation_step(batch, batch_idx)
            outfea = outputs.view(bs, -1)
            all_outfea = all_outfea + (outfea,)
            res = eps - outputs
            all_loss = all_loss + (loss,)
            all_prob = all_prob + (res.view(bs, -1),)
            all_logit = all_logit + (outputs.view(bs, -1),)
    ...
    return (losses.avg, None, all_loss, None, all_logit, all_outfea, None, all_prob)
...
def compute_sims(data, sigma=1.0, metric='l2', fl_alpha=2., fl_beta=0.75, fl_gamma=2., fl_lambda=0.9, fl_gsrow=False,
                 fl_cent_op=True, fl_cent_mn=False, fl_cent_sm=False):
...
def compute_feamat(feature_matrix, fe_alpha=2., fe_beta=0.75, fe_gamma=2., fe_lambda=0.9, fe_gsrow=False, fe_gsmin=True,
                   fe_cent_op=True, fe_cent_mn=False, fe_cent_sm=False, fe_use_entropy_centrality=False):
    ...
    if fe_lambda < 1.0:
        ...
        if fe_use_entropy_centrality:
            ...
            for ii in range(feature_matrix.shape[0]):
                fe_centrality[ii] = max_entropy - entropy(
                    feature_matrix[ii].T)  # Not sure how to correct it. For now, just disable use_centrality.
...
def adjust_cosine_learning_rate(optimizer, epoch, schedule_idx):
    ...
    state['lr'] = args.lr_min + (args.lr_max - args.lr_min) * 0.5 * (
            1 + np.cos((float(step) / float(total_steps)) * np.pi))
    args.lr = state['lr']
